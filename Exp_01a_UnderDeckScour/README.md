# Bridge Pressure-Flow Scour — Case Exp-01a (Soffit Scour)
Source: *Journey to the Correct Result* (Subhadip Das, HWRE Lab, IITK, 2016)

This case simulates the bridge deck pressure-flow scour under **Exp-01a** conditions using the two-phase sediment transport solver `sedFoam_rbgh` in OpenFOAM 2412.
...
| :--- | :--- | :--- |
| **Water depth ($Y$)** | $0.12\text{ m}$ | Free-surface elevation (Rigid Lid) |
| **Approach velocity ($U$)** | $0.23\text{ m/s}$ | Mean inlet velocity |
| **Discharge ($Q$)** | $8.28\text{ L/s}$ | Volumetric flow rate |
...
| **Scour Location** | **Under the bridge** | Expected experimental result |
...
$$\text{soffit\_y} = Y - H_b = 0.12\text{ m} - 0.072\text{ m} = 0.048\text{ m}$$

*   **Domain Top ($y_{\text{top}}$):** $0.12\text{ m}$ (Water Surface datum)
*   **Soffit Elevation ($y_{\text{soffit}}$):** $0.048\text{ m}$
...
```
   y-levels:
   L3 (Y = 0.12m)            12─────────────13             14─────────────15
                             │   Block 6   │  [ NO MESH ]  │   Block 7   │  (Solid Bridge Deck)
   L2 (y = 0.048m, Soffit)    8──────────────9─────────────10─────────────11
...
*   **`inlet`**: Left boundaries representing incoming flow with a logarithmic velocity profile starting at $0$ at the bed interface ($y = 0.0\text{ m}$) and scaling to $V_a = 0.23\text{ m/s}$ at the water surface ($y = 0.12\text{ m}$).
...
*   **`bridgeDeck`**: Wall boundary representing the physical deck:
    *   **Upstream Wall:** Vertical face at $x = 0.0\text{ m}, y \in [0.048, 0.12]\text{ m}$
    *   **Soffit:** Horizontal face at $y = 0.048\text{ m}, x \in [0.0, 0.153]\text{ m}$
    *   **Downstream Wall:** Vertical face at $x = 0.153\text{ m}, y \in [0.048, 0.12]\text{ m}$
